...
app.post('/analyze', async function(req, res) {
  ...
  try {
    ...
    content.push({ type: 'text', text: 'You are a professional book reseller. These photos show a book for eBay. IMPORTANT: The LAST image is a handwritten note card — read it to extract condition, shelf location, and weight (lbs and oz). All other images are book photos. Reply ONLY with raw JSON, no markdown:\n{"title":"Full Title","author":"Author Name or Unknown","bookTitle":"Title Only","format":"Hardcover or Paperback or Trade Paperback","language":"English","description":"2-3 sentences describing the book","genre":"Fiction or Nonfiction or Mystery etc","publisher":"Publisher Name or unknown","publicationYear":"YYYY or unknown","isbn":"ISBN if visible or unknown","topic":"main subject/topic","condition":"condition text from note card","shelfLocation":"shelf location from note card","weightLbs":"number or 0","weightOz":"number or 0","minPrice":5,"maxPrice":25,"avgPrice":12,"suggestedPrice":10}' });
    var r = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
...
app.get('/', function(req, res) {
  ...
  h += '      condition:"",shelfLocation:"",weightLbs:"",weightOz:"",\n';
  ...
  h += '      it.isbn=p.isbn||"";it.topic=p.topic||"";\n';
  h += '      it.condition=p.condition||"";it.shelfLocation=p.shelfLocation||"";\n';
  ...
  h += '        condition:it.condition,shelfLocation:it.shelfLocation,\n';
