...
app.get('/', function(req, res) {
  ...
  h += '.toast-wrap{position:fixed;bottom:20px;right:20px;z-index:9999;display:flex;flex-direction:column;gap:8px;max-width:340px}';
  ...
  h += '      condition:"Good",price:10,min:5,max:20,avg:12,weightLbs:"",weightOz:""});\n';
  h += '  });\n';
  ...
  h += '        isbn:item.isbn,topic:item.topic,price:item.price,condition:item.condition,\n';
  ...
  // Toast
  h += 'function toast(msg,type){var w=document.getElementById("toasts"),t=document.createElement("div");t.className="toast"+(type?" "+type:"");t.textContent=msg;w.appendChild(t);setTimeout(function(){t.remove()},6000)}\n';
