...
app.post('/post-listing', async function(req, res) {
  ...
  try {
    ...
    // Weight and package size for shipping
    var weightXml = '';
    var pkgDims = { 'Small (6x4x2)': [6,4,2], 'Medium (9x6x3)': [9,6,3], 'Large (12x9x4)': [12,9,4], 'Oversized (14x11x6)': [14,11,6] };
    var dims = pkgDims[listing.pkgSize] || [9,6,3];
    // Convert decimal lbs to lbs+oz (e.g. 1.5 lbs = 1 lb 8 oz)
    var totalOz = Math.round((parseFloat(listing.weightLbs) || 0) * 16);
    ...
    weightXml = '<ShippingPackageDetails>' +
      ...
      '<PackageLength unit="in">' + dims[0] + '</PackageLength>' +
      '<PackageWidth unit="in">' + dims[1] + '</PackageWidth>' +
      '<PackageDepth unit="in">' + dims[2] + '</PackageDepth>' +
...
app.get('/', function(req, res) {
  ...
  h += '    ["Small (6x4x2)","Medium (9x6x3)","Large (12x9x4)","Oversized (14x11x6)"].forEach(function(s){b+="<option"+(item.pkgSize===s?" selected":"")+">"+s+"</option>";});\n';
  h += '    b+="</select></div>";\n';
  h += '    b+="</div>";\n';
  ...
  h += '        weightLbs:item.weightLbs,weightOz:item.weightOz,pkgSize:item.pkgSize},\n';
  h += '      images:images})})\n';
  h += '    .then(function(r){return r.json()})\n';
  h += '    .then(function(d){\n';
