...
app.get('/', function(req, res) {
  ...
  h += '<div class="tip">📋 <strong>How to use:</strong> Between each book, take a photo of a <strong>blank white piece of paper or sticky note</strong> — the app detects it and starts a new book automatically. No timing needed!</div>';
  h += '</div>';
  ...
  // Core grouping logic — sort by lastModified, split on NOTE photos
  h += 'function looksLikeNote(filename){';
  h += 'var n=(filename||"").toLowerCase();';
  h += 'return /note|divid|next|split|break|new.?book|separator|sep|marker/.test(n)}';

  h += 'function isLikelyBlank(canvas){';
  h += 'var ctx=canvas.getContext("2d"),d=ctx.getImageData(0,0,canvas.width,canvas.height).data,bright=0;';
  h += 'for(var i=0;i<d.length;i+=16)bright+=(d[i]+d[i+1]+d[i+2])/3;';
  h += 'return(bright/(d.length/16))>210}'; // very bright = white paper / note

  h += 'function checkIsNote(file){';
  h += 'return new Promise(function(resolve){';
  h += 'if(looksLikeNote(file.name)){resolve(true);return}';
  h += 'var img=new Image(),url=URL.createObjectURL(file);';
  h += 'img.onload=function(){';
  h += '  var c=document.createElement("canvas");c.width=80;c.height=80;';
  h += '  c.getContext("2d").drawImage(img,0,0,80,80);';
  h += '  URL.revokeObjectURL(url);';
  h += '  resolve(isLikelyBlank(c))};';
  h += 'img.onerror=function(){URL.revokeObjectURL(url);resolve(false)};';
  h += 'img.src=url})}';
  ...
  h += 'document.getElementById("gapInfo").textContent="Scanning "+imgs.length+" photos for note dividers...";';
  // Check each photo — is it a note/divider?
  h += 'var checks=imgs.map(function(f){return checkIsNote(f)});';
  h += 'Promise.all(checks).then(function(isNote){';
  h += '  var groups=[],cur=[];';
  h += '  var noteCount=0;';
  h += '  for(var i=0;i<imgs.length;i++){';
  h += '    if(isNote[i]){'; // this photo is a note — start new group
  h += '      if(cur.length>0)groups.push(cur);';
  h += '      cur=[];noteCount++;';
  h += '    } else {';
  h += '      cur.push(imgs[i]);';
  h += '      if(cur.length>=12){groups.push(cur);cur=[];}'; // safety max 12
  h += '    }';
  h += '  }';
  h += '  if(cur.length)groups.push(cur);';
  // fallback: if no notes found, use 30s gap
  h += '  if(noteCount===0){';
  h += '    groups=[];cur=[];';
  h += '    for(var i=0;i<imgs.length;i++){';
  ...
  h += '    }';
  h += '    if(cur.length)groups.push(cur);';
  h += '    document.getElementById("gapInfo").textContent="No note dividers found — used 30s time gap instead. "+imgs.length+" photos → "+groups.length+" books";';
  h += '  } else {';
  h += '    document.getElementById("gapInfo").textContent="Found "+noteCount+" note dividers! "+imgs.length+" photos → "+groups.length+" books";';
  h += '  }';
  h += '  groups=groups.filter(function(g){return g.length>0});';
