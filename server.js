...
console.log('Starting FlipAI Bookslayer (Gemini Edition)...');
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', service: 'FlipAI Bookslayer', ai: 'Gemini' });
});
...
app.post('/verify', async function(req, res) {
  ...
  try {
    ...
    const model = testClient.getGenerativeModel({ model: 'gemini-2.5-flash-lite' }, { apiVersion: 'v1beta' });
...
// ─── ANALYZE BOOK PHOTOS — Step 1: Gemini vision, Step 2: 1st ed web search ──
app.post('/analyze', async function(req, res) {
  ...
  try {
    // Use key from request (UI) or fall back to env var
    const { GoogleGenerativeAI } = require('@google/generative-ai');
    ...
    var model = gemini.getGenerativeModel(
      { model: 'gemini-2.5-flash-lite', generationConfig: { responseMimeType: 'application/json' } },
      { apiVersion: 'v1beta' }
    );
    ...
    var r1 = await model.generateContent(parts);
    var t1 = r1.response.text().replace(/```json|```/g, '').trim();
    ...
    // STEP 2 — 1st edition: live web search via Gemini grounding. Standard: AI estimate only.
    ...
    if (isFirstEd) {
      try {
        ...
        var pricePrompt = 'You are an expert rare/collectible book reseller. Search for FIRST EDITION pricing only — ignore all regular edition prices. Return ONLY raw JSON, no markdown.\n\n' +
          ...
          '1. "' + title + '" "' + author + '" "first edition" price value — AbeBooks, PriceCharting, collector guides\n' +
          '2. "' + title + '" first edition used book price abebooks OR bookscouter OR pricecharting\n' +
          '3. "' + title + '" "' + author + '" first edition ebay sold 2024 2025\n' +
          '4. "' + title + '" first printing collectible value\n' +
          '\nIMPORTANT: Only use prices clearly marked as first edition or first printing.\n' +
          'PRICING FORMULA:\n' +
          '- sweetSpot = ebaySoldAvg × 0.92 (undercut by 8%)\n' +
        ...
        var searchModel = gemini.getGenerativeModel(
          { model: 'gemini-2.5-flash-lite', tools: [{ googleSearch: {} }] },
          { apiVersion: 'v1beta' }
        );
        var r2 = await searchModel.generateContent(pricePrompt);
        var t2 = r2.response.text().replace(/```json|```/g, '').trim();
    ...
    // Return in same shape the client expects
  ...
  try {
    var uploadPromises = images.slice(0, 12).map(function(img) {
      return uploadPhotoToEbay(img.data, img.mimeType, appId, token).catch(function(e) {
        console.log('Photo upload skipped:', e.message); return null;
      });
    });
    ...
    var xml = '<?xml version="1.0" encoding="utf-8"?>' +
      ...
      '<Description><![CDATA[' + ((listing.firstEdition === 'Yes' ? 'First Edition. ' : '') + (listing.description || 'See pictures for condition.')) + ']]></Description>' +
      pictureXml +
...
app.get('/', function(req, res) {
  ...
  h += '<label>Gemini API Key</label><input type="password" id="apiKey" placeholder="AIzaSy...">';
  ...
  h += '  if(!k){setStatus("Enter your Gemini API key","err");return}\n';
  h += '  setStatus("Testing...","");\n';
  ...
  h += '    setStatus("Gemini API working! eBay App ID saved. Ready to analyze.","");\n';
  h += '  })\n';
  ...
  h += '  b+="<div class=\'card-meta\'>Book #"+num+" &bull; "+item.files.length+" photo"+(item.files.length>1?"s":"")+" &bull; "+item.status+"<button onclick=\'deleteItem("+item.id+")\' style=\'float:right;background:none;border:none;color:#6b6b8a;font-size:1.1rem;cursor:pointer;line-height:1\' title=\'Delete\'>&times;</button></div>";\n';
  ...
  h += 'function deleteItem(id){items=items.filter(function(i){return i.id!=id});render();updateStats()}\n';
  h += 'function mergeWithNext(id){\n';
  h += '  var idx=items.findIndex(function(i){return i.id==id});\n';
  h += '  if(idx<0||idx>=items.length-1){toast("No next card to merge with","");return}\n';
  h += '  var a=items[idx],b=items[idx+1];\n';
  h += '  a.files=a.files.concat(b.files);\n';
  h += '  a.urls=a.urls.concat(b.urls);\n';
  h += '  a.status="idle";a.title="";\n';
  h += '  items.splice(idx+1,1);\n';
  h += '  render();updateStats();toast("Merged — re-analyze the card","")\n';
  h += '}\n';
